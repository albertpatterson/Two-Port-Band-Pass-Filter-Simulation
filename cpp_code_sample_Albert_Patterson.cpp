...
class TwoPortNetwork{
    ...
    public:
        //construct and initialize parameters as transmission parameters based
        // on provided circuit information
...
// calculate the real part of (r1+j*i1)*(r2+j*i2)=(r1*r2-i1*i2)+j*(r1*i2+r2+i1)
// where j=sqrt(-1)
double TwoPortNetwork::calcComplexProductReal(double real_val_1,double imag_val_1,
                              double real_val_2, double imag_val_2){
...
// where j=sqrt(-1)
double TwoPortNetwork::calcComplexProductImag(double real_val_1,double imag_val_1,
                              double real_val_2, double imag_val_2){
...
// (r1+j*i1)/(r2+j*i2)=((r1*r2+i1*i2)/(r2^2+i2^2))+j*((r2*i1-r1+i2)/(r2^2+i2^2))
// where j=sqrt(-1) and r1, r2, i1, and i2 are real numbers
double TwoPortNetwork::calcComplexQuotientReal(double real_val_1,double imag_val_1,
                               double real_val_2, double imag_val_2){
...
// (r1+j*i1)/(r2+j*i2)=((r1*r2+i1*i2)/(r2^2+i2^2))+j*((r2*i1-r1+i2)/(r2^2+i2^2))
// where j=sqrt(-1) and r1, r2, i1, and i2 are real numbers
double TwoPortNetwork::calcComplexQuotientImag(double real_val_1,double imag_val_1,
                               double real_val_2, double imag_val_2){
...
// transmission matrix values assigned depending on the frequency and component
// values
...
void TwoPortNetwork::convert_params_to_s(){
    if(param_type_=='T'){
        ...
        for(int row=0; row<n_freqs_;row++){
            ...
            // s12 = s12_num/den
            s12_real=calcComplexQuotientReal(s12_num_real,s12_num_imag,den_real,
                                             den_imag);
            s12_imag=calcComplexQuotientImag(s12_num_real,s12_num_imag,den_real,
                                             den_imag);
            ...
            s21_num_imag=0;
            //s21 =s21_num/den
            s21_real=calcComplexQuotientReal(s21_num_real,s21_num_imag,den_real,
                                             den_imag);
            s21_imag=calcComplexQuotientImag(s21_num_real,s21_num_imag,den_real,
                                             den_imag);
            ...
            // s22=s22_num/den
            s22_real=calcComplexQuotientReal(s22_num_real,s22_num_imag,den_real,
                                             den_imag);
            s22_imag=calcComplexQuotientImag(s22_num_real,s22_num_imag,den_real,
                                             den_imag);
...
double TwoPortNetwork::get_insertion_loss(){
        ...
        return -1*insertion_loss;// report insertion loss as positive (convention)
}

// calculate the center frequency of a two port network
double TwoPortNetwork::get_center_frequency(){
        double* s21dbs=this->get_s21_dB();// get |S21|
        int center_frequency_index=findMaxIndex(s21dbs,n_freqs_);//find max |S21|
...
void TwoPortNetwork::cascade(TwoPortNetwork& tpn_b){
    ...
    double temp_param_values[8];// temporarily hold new T values of cascded tpn
    ...
    if ((param_type_='T')&&(tpn_b.get_param_type()=='T')){
        for(int row=0;row<n_freqs_;row++){
            ...
            temp_param_values[0+1]=calcComplexProductImag(
                                        ...
                                        //imaginary part b11
                                        tpn_b.param_values_[0+1][row])
                                +calcComplexProductImag(//imaginary part a12*b21
                                        //real part a12
                                        param_values_[2][row],
                                        //imaginary part a12
                                        param_values_[2+1][row],
            ...
            temp_param_values[2]=calcComplexProductReal(//real part of a11*b12
                                        //real part of a11
                                        param_values_[0][row],
                                ...
                                +calcComplexProductReal(//real part of a12*b22
                                        // real part of a12
										param_values_[2][row],
            ...
            temp_param_values[4]=calcComplexProductReal(// real part of a21*b11
                                        // real part of a21
										param_values_[4][row],
										...
										tpn_b.param_values_[0+1][row])
                                +calcComplexProductReal(// real part of a22*b21
                                        // real part of a22
										param_values_[6][row],
            ...
            temp_param_values[6]=calcComplexProductReal(// real part of a21*b12
                                        // real part of a21
										param_values_[4][row],
                                        // imaginary part of a21
										param_values_[4+1][row],
                                        ...
                                        // imaginary part of b12
										tpn_b.param_values_[2+1][row])
                                +calcComplexProductReal(// real part of a22*b22
                                        // real part of a22
										param_values_[6][row],
                                        // imaginary part of a22
										param_values_[6+1][row],
...
//        o------------------------------------------------------------------------------------------o
	...
	TwoPortNetwork static_capacitor(n_freqs, freqs,'c',static_capacitance_val,
									false);
    //two port network for the motional resistance
	TwoPortNetwork motional_resistor(n_freqs, freqs,'r',motional_resistance_val,
									true);
    //two port network for the motional inductance
	TwoPortNetwork motional_inductor(n_freqs, freqs,'l',motional_inductance_val,
									true);
...
bool saveSData(){
    ...
    bool save_s=false;// boolean value for saving s parameter data default false
